...
for p in targets:
    ...
    # Inject HTML into Tab 2 if not already present
    if "Active Lineal Roster & Access Control" not in html:
        # Insert right before the closing tag of tab-panel-2
        html = html.replace('<!-- TAB 3: Velocity Radar & Scanner -->', roster_section_html + '\n  <!-- TAB 3: Velocity Radar & Scanner -->')

    # Inject JavaScript poller hooks
    if "syncRoster" not in html:
        html = html.replace('function syncData() {', roster_js + '\n    function syncData() {')
        html = html.replace('syncData();', 'syncData();\n      syncRoster();')
        html = html.replace('setInterval(syncData, 3000);', 'setInterval(syncData, 3000);\n      setInterval(syncRoster, 5000);')

    p.write_text(html, encoding="utf-8")
    print(f"Mounted Live Provisioning & Promotion UI in: {p}")

print("Tab 2 UI update complete.")