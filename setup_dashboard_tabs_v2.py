...
active_nav_html = '''<div id="tab-nav" style="display:flex; gap:10px; margin-bottom:20px;">
...
for file_path in [pathlib.Path("backend/app/api/v1/dashboard.html"), pathlib.Path("backend/app/static/dashboard.html")]:
    ...
    # 1. Clean out any previous partial panel injections to ensure fresh state
    content = re.sub(r'<!-- TAB 2: Family Lineal Pools.*?<!-- TAB 4: Governance & Dual-Control -->.*?</div>\s*</div>\s*</div>', '', content, flags=re.DOTALL)
    content = content.replace('<div id="tab-panel-1">', '').replace('function switchTab', 'function _oldSwitchTab')

    # 2. Locate the tab bar using exact button text anchors (immune to CSS whitespace)
    idx_btn1 = content.find("1. Founder SCMA")
    idx_btn4 = content.find("4. Governance & Dual-Control")
    if idx_btn1 != -1 and idx_btn4 != -1:
        start_nav = content.rfind("<div", 0, idx_btn1)
        end_nav = content.find("</div>", idx_btn4) + len("</div>")
        content = content[:start_nav] + active_nav_html + content[end_nav:]

    # 3. Wrap Tab 1 overview content and inject Tabs 2, 3, 4 panels
    # Tab 1 starts right after active_nav_html and ends after ACTIVE PORTFOLIO POSITIONS table
    pos_idx = content.find("ACTIVE PORTFOLIO POSITIONS")
    if pos_idx != -1:
        tbl_end = content.find("</table>", pos_idx)
        if tbl_end != -1:
            end_tab1 = content.find("</div>", tbl_end) + len("</div>")
            nav_end_idx = content.find(active_nav_html) + len(active_nav_html)
            
            tab1_content = content[nav_end_idx:end_tab1].strip()
            wrapped_body = f'\n    <div id="tab-panel-1">\n{tab1_content}\n    </div>\n{additional_panels}\n'
            content = content[:nav_end_idx] + wrapped_body + content[end_tab1:]

    # 4. Inject switchTab JavaScript function before </body>
    content = content.replace('</body>', js_tab_switch + '\n</body>')

    file_path.write_text(content, encoding="utf-8")
    
    # Verify patch integrity
    check_btn = "id=\"tab-btn-1\"" in content
    check_p1 = "id=\"tab-panel-1\"" in content
    check_p2 = "id=\"tab-panel-2\"" in content
    check_js = "function switchTab" in content
    print(f"Patched: {file_path}")
    print(f"  -> Tab Buttons Active: {check_btn}")
    print(f"  -> Tab Panel 1 Wrapped: {check_p1}")
    print(f"  -> Tab Panel 2 Injected: {check_p2}")
    print(f"  -> switchTab JS Active: {check_js}")
    files_patched += 1

print(f"\nCompleted patching {files_patched} dashboard file(s).")