...
class MarketDepthEngine:
    def __init__(self, fee_rate: float = 0.01, max_slippage_pct: float = 0.015):
    ...
    def evaluate_order_depth(self, order_book: List[Dict[str, Any]], requested_volume: float, max_slippage_pct: Optional[float] = None) -> Dict[str, Any]:
        ...
        if total_filled < requested_volume:
            return {
                'executable': False,
                'executed_volume': total_filled,
                'vwap': round(total_cost / total_filled, 4) if total_filled > 0 else 0.0,
                'slippage': 0.0,
    ...
    def evaluate_net_edge_friction(self, raw_edge: float, spread: float, fee_rate: Optional[float] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            'friction': friction,
            'net_edge': net_edge,
            'reason': 'PROFITABLE_NET_EDGE' if admissible else 'FRICTION_ELIMINATES_EDGE'
        }
