...
for file_path in [pathlib.Path("backend/app/api/v1/dashboard.html"), pathlib.Path("backend/app/static/dashboard.html")]:
    ...
    # 1. Clean out any previous partial attempts
    content = re.sub(r'<div id="tab-panel-1">', '', content)
    content = re.sub(r'</div><!-- /tab-panel-1 -->', '', content)
    content = re.sub(r'<!-- TAB 2: Family Lineal Pools.*?<!-- TAB 4: Governance & Dual-Control -->.*?</div>\s*</div>\s*</div>', '', content, flags=re.DOTALL)

    # 2. Find the exact end of the tab buttons container
    btn4_idx = content.find('id="tab-btn-4"')
    if btn4_idx == -1:
        print(f"Error: tab-btn-4 not found in {file_path}")
        continue
    nav_end = content.find("</div>", btn4_idx) + len("</div>")

    # 3. Find the end of the main dashboard wrapper (immediately preceding copilot/orc markup)
    marker = "<!-- ==================== PDEUE COPILOT"
    copilot_idx = content.find(marker)
    if copilot_idx == -1:
        copilot_idx = content.find('id="copilot-drawer"')
    if copilot_idx == -1:
        copilot_idx = content.find('</body>')

    content_end = content.rfind("</div>", 0, copilot_idx)

    # 4. Extract Tab 1 content, wrap it, and append Panels 2, 3, 4
    tab1_inner = content[nav_end:content_end].strip()
    wrapped_content = f'\n    <div id="tab-panel-1">\n{tab1_inner}\n    </div><!-- /tab-panel-1 -->\n{additional_panels}\n  '
    content = content[:nav_end] + wrapped_content + content[content_end:]

    file_path.write_text(content, encoding="utf-8")

    # Verify injection results
    print(f"Patched: {file_path}")
    print(f"  -> Tab Buttons Active: {'id=\"tab-btn-1\"' in content}")
    print(f"  -> Tab Panel 1 Wrapped: {'id=\"tab-panel-1\"' in content}")
    print(f"  -> Tab Panel 2 Injected: {'id=\"tab-panel-2\"' in content}")
    print(f"  -> switchTab JS Active: {'function switchTab' in content}")

print("Dashboard tab panels successfully wired.")