...
# Complete, self-contained UI & JS bundle with no quote collisions or syntax errors
copilot_bundle = '''
<!-- ==================== PDEUE COPILOT & ORC RUNTIME ==================== -->
...
<div id="copilot-drawer" style="position:fixed; bottom:25px; right:25px; width:400px; height:520px; background:#0f172a; border:2px solid #38bdf8; border-radius:8px; box-shadow:0 10px 25px rgba(0,0,0,0.6); z-index:9999; display:flex; flex-direction:column; overflow:hidden;">
  ...
  <div style="padding:12px 16px; background:#1e293b; border-top:1px solid #334155; display:flex; gap:8px;">
    ...
    <button id="copilot-send-btn" onclick="sendCopilotQuery()" style="background:#38bdf8; color:#0f172a; font-weight:bold; border:none; border-radius:4px; padding:8px 14px; cursor:pointer; font-size:0.85rem;">Send</button>
  </div>
</div>
...
function executeActionCard(cardId) {
  ...
  if (!card) {
    console.warn("Card not found in registry:", cardId);
    return;
  }
  ...
  alert('Action executed under AUTH-01: ' + card.title);
}
</script>
<!-- ==================== END PDEUE COPILOT & ORC ==================== -->
'''
...
for file_path in [pathlib.Path("backend/app/api/v1/dashboard.html"), pathlib.Path("backend/app/static/dashboard.html")]:
    ...
    # Strip any previous broken injected Copilot/ORC blocks cleanly
    content = re.sub(r'<!-- ==================== PDEUE COPILOT & ORC RUNTIME ==================== -->.*?<!-- ==================== END PDEUE COPILOT & ORC ==================== -->', '', content, flags=re.DOTALL)
    content = re.sub(r'<!-- Floating Copilot Launcher.*?<!-- Opportunity Research Center.*?</div>\s*</div>', '', content, flags=re.DOTALL)
    content = re.sub(r'<div id="copilot-drawer".*?</div>\s*</div>', '', content, flags=re.DOTALL)
    content = re.sub(r'<div id="orc-drawer".*?</div>\s*</div>', '', content, flags=re.DOTALL)
    content = re.sub(r'<script>\s*window\._actionCardRegistry.*?</script>', '', content, flags=re.DOTALL)
    content = re.sub(r'function clearChat\(.*?</script>', '</script>', content, flags=re.DOTALL)

    # Insert clean bundle right before </body>
    content = content.replace('</body>', copilot_bundle + '\n</body>')
    file_path.write_text(content, encoding="utf-8")
    print(f"Restored clean interface in: {file_path}")
